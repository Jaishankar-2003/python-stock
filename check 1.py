...
def extract_data_from_text(text):
    ...
    return {
        "volume": extract_float(r"Traded Volume .*?([\d,.]+)"),
        "value": extract_float(r"Traded Value .*?([\d,.]+)"),
        "cap": extract_float(r"Total Market Cap .*?([\d,.]+)"),
        "float_cap": extract_float(r"Free Float Market Cap .*?([\d,.]+)"),
        "impact": extract_float(r"Impact cost\s*([\d.]+)"),
        "delivery": extract_percent(r"Deliverable / Traded\s+Quantity\s+([\d.]+)\s*%"),
        "margin": extract_float(r"Applicable Margin Rate\s*([\d.]+)"),
        "daily_vol": extract_float(r"Daily Volatility\s*([\d.]+)"),
        "annual_vol": extract_float(r"Annualised Volatility\s*([\d.]+)"),
        "pe": extract_float(r"Adjusted P/E\s*([\d.]+)"),
        "low52": extract_float(r"52 Week Low.*?([\d.]+)"),
        "high52": extract_float(r"52 Week High.*?([\d.]+)"),
        "index": extract_string(r"Index\s+(.*?)\n")
    }

# --- Market Cap Segment Logic ---
...
# --- GUI Setup ---
root = tk.Tk()
root.title("Swing Trade Evaluator")
...
# Buttons and Output
tk.Button(root, text="Auto-Fill From Raw Data", command=populate_fields).grid(row=len(fields), column=0, columnspan=2, pady=5)
tk.Button(root, text="Evaluate", command=evaluate_stock).grid(row=len(fields)+1, column=0, columnspan=2, pady=5)
...
input_box.grid(row=len(fields)+5, column=0, columnspan=2, padx=10, pady=5)
